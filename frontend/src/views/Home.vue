...
const loadDashboard = async () => {
  try {
    const [analysisResponse, reviewResponse, holdingResponse, activityResponse, indexResponse] = await Promise.all([getAllAnalysis(), getMarketReviews(), getPortfolioHoldings(), getMarketActivity(), getMarketIndex()])
    ...
    marketActivity.value = activityResponse.data?.data || null
    marketIndex.value = indexResponse.data?.data || null
...
onMounted(() => { loadDashboard(); if (isTradingTime()) marketTimer = window.setInterval(loadDashboard, 20000) })
