...
comment on column audit_log.ip_address is '请求IP';
...
create index idx_audit_log_create_time
...
(
    id             bigint    default nextval('usage_stats_id_seq'::regclass) not null
        primary key,
    user_id        bigint                                                    not null,
...
comment on table usage_stats is '使用统计表';

comment on column usage_stats.id is '主键ID';

comment on column usage_stats.user_id is '用户ID';

comment on column usage_stats.stat_date is '统计日期';

comment on column usage_stats.analysis_count is '当日评估次数';

comment on column usage_stats.review_count is '当日复盘次数';

comment on column usage_stats.create_time is '创建时间';

comment on column usage_stats.update_time is '更新时间';

comment on column usage_stats.version is '版本号（乐观锁）';

comment on column usage_stats.creator is '创建人';

comment on column usage_stats.modifier is '修改人';

alter table usage_stats
    owner to postgres;

create unique index idx_usage_stats_user_date
    on usage_stats (user_id, stat_date);

create table system_role
(
    id          bigint      default nextval('system_role_id_seq'::regclass) not null
        primary key,
...
(
    id              bigint      default nextval('system_menu_id_seq'::regclass) not null
        primary key,
    parent_id       bigint,
...
(
    id              bigint      default nextval('system_api_id_seq'::regclass) not null
        primary key,
...
(
    id          bigint    default nextval('system_role_menu_id_seq'::regclass) not null
        primary key,
...
(
    id          bigint    default nextval('system_role_api_id_seq'::regclass) not null
        primary key,
    role_id     bigint                                                        not null,
...
(
    id          bigint    default nextval('system_menu_api_id_seq'::regclass) not null
        primary key,
    menu_id     bigint                                                        not null,
    api_id      bigint                                                        not null,
    create_time timestamp default now()                                       not null,
...
(
    id          bigint    default nextval('system_user_role_id_seq'::regclass) not null
        primary key,
    user_id     bigint                                                         not null,
    role_id     bigint                                                         not null,
    create_time timestamp default now()                                        not null,
